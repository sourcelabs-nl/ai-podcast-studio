...
- **Every call's options must carry the stage timeout.** Since Spring AI 2.0.1, `OpenAiChatModel` sends the options' timeout per request, and an `OpenAiChatOptions` without one defaults to 60s, overriding the timeout the `OpenAIClient` was built with. `withRoutingAndReasoning(resolvedModel)` sets it from `ResolvedModel.requestTimeout`; an options builder that skips that call gets the 60s cap. See `knowledge/models-and-apis/spring-ai-2-0-1-per-request-timeout.md`.
- **Never send `temperature` to an OpenAI reasoning model.** `openai/*` models (probed on `openai/gpt-6-luna`) do not list `temperature` among their supported parameters. Combined with the `require_parameters` routing this project always sends, a request carrying one matches no endpoint and OpenRouter answers 404 "No endpoints found that can handle the requested parameters", which reads like a routing problem rather than a parameter one. Build options through `withRoutingAndReasoning(resolvedModel)`, which clears the temperature for such models via `OpenRouterRouting.acceptsTemperature`; do not set a temperature after that call. See `knowledge/models-and-apis/openai-reasoning-models-reject-temperature.md`.
