...
Every model call in the app is billed to one of four stages. A stage is the unit of model choice,
cost and timeout: each one resolves its own model per podcast (falling back to `app.llm.defaults`),
gets its own request timeout, and reports its own token usage and cost. The first three produce an
...
`DEDUP` makes one call that is not a stage of its own: a Jev decision that drops the candidates
recent episodes already covered, before the clustering model is asked to sort what is left. It has
its own model and timeout but no separate budget or billing line, and its charge is added to the
`DEDUP` total, because it exists only to make that stage's call smaller. Step 3 below describes what
it does and what happens when it is unavailable.
...
A second, smarter language model writes the actual episode. Before it starts, the app groups today's articles by topic and compares them against recent episodes: brand-new topics go in fresh, topics that follow up on something covered earlier get a "follow-up" hint so the script can naturally reference previous coverage, and topics you already covered to death get skipped. 

Ahead of that grouping call sits [Jev](https://typesafe.ai/blog/introducing-system-one-models-and-jev), TypeSafe's decision model, reached through the same OpenRouter key the rest of the pipeline uses. It answers one question about each candidate: is this about something recent episodes already covered? Jev does not generate text at all. It returns a probability against a schema, so it cannot answer with anything the app fails to read, and everything it marks as already covered is dropped before the grouping model sees it. On a real run that removed a quarter of the candidates in under two seconds and cut the whole step from thirty-six seconds to twelve, because the grouping call also gets a smaller list to work through. It is an optimisation and behaves like one: it asks all its questions in a single request, retries a request the endpoint was too busy to serve, and if it answers nothing at all the grouping call simply sees every candidate, exactly as it did before. It is also not allowed to end a run on its own. If it were to mark every candidate as covered, that verdict is overruled and the full list goes to the grouping model anyway, because an episode being skipped for lack of new material is a conclusion that belongs to the model that can actually weigh the stories.

Before the writer is called, a research stage looks up prior coverage in a full-text index of all your past episodes and, when deep-dive research is enabled (or the episode is a focus episode, see below), runs real web searches via Tavily for extra context on the day's standout stories. Both are handed to the writer in its prompt rather than as tools it can call, so the write itself completes in a single model request. The opening, transitions, sign-off, and other patterns get rotated automatically so episodes don't all sound the same. The script is written for the day the episode covers, not the day the run happens: the date it announces, whether it gets the extra end-of-week beat, and which patterns it rotates into all come from the end of the episode's window, read in the podcast's timezone. So a re-run or a regeneration of Wednesday's episode still opens as Wednesday's episode, however much later you start it.
...
judge score, cost, generation time, reasoning tokens and the provider that served each call. Each
run also gets a compose breakdown from OpenRouter's own account of its requests: startup (until the
provider began responding), reasoning, writing, reasoning share, output tokens per second and the
number of provider fallbacks, so a slow run shows whether it queued, reasoned at length or was served
by a slow endpoint.
...
The Jev gate described in [Step 3](#step-3-writing-the-script) is reached through OpenRouter and takes no key of its own. Without an OpenRouter credential, on `openai` or `ollama` for instance, it finds nothing to authenticate with, logs that it is asking nothing, and the dedup stage runs exactly as it did before the gate existed.
...
- **Episode detail page**, dedicated page per episode with tabs for Script (chat-bubble rendering, each turn labelled with its index), Articles (a table of the episode's sources, each row carrying the article count and the best relevance score among them and expanding in place to the articles themselves; an article aggregated from several posts shows its thread size and expands to the individual posts), Publications, **Costs** (per-stage breakdown: scoring, dedup, the dedup gate, compose, recap, TTS, research, plus total, ordered dearest first), and **Evaluation** (the episode's attention score and its components, each shown as a bar coloured by its own value rather than against a pass mark, the script's shape per speaker, the judge's anchors and the metrics' outliers, and the conditions the script was composed under). Every turn index in the Evaluation tab jumps to that turn in the Script tab and highlights it, so a figure can be read against the line that produced it. Shows episode metadata, recap, inline audio player, and contextual action buttons (Approve, Discard, Publish, Regenerate, Regenerate Audio, Retry, Regenerate Recap)
...
The episode workflow is: `PENDING_REVIEW` → (edit script if needed) → `APPROVED` → `GENERATING_AUDIO` (TTS in progress) → `GENERATED`. The `GENERATING_AUDIO` status is persisted in the database so the UI shows a "Generating audio..." spinner across page reloads; on app startup any stale `GENERATING_AUDIO` episodes are recovered as `FAILED`. You can also discard an episode in `PENDING_REVIEW`, `GENERATED`, or `FAILED` status (discarding resets non-aggregated articles so they are included in the next generation run, while aggregated articles from X/Nitter sources are deleted so their posts get re-aggregated fresh with any new posts on the next run). An episode with any publication in `PUBLISHED` status refuses discard outright, naming the published targets, so a live episode can never be pulled out from under its publications; unpublish it first. Articles linked to published episodes are never reset or deleted during discard, preventing published content from being reprocessed.
...
Episode responses include token usage and costs broken down per pipeline stage: **Scoring**, **Dedup**, **Compose**, **Recap**, **TTS**, and **Research**, plus the number of TTS synthesis calls an episode made. For an episode whose requests are individually attributed to it, each stage's totals are the sum of that episode's recorded LLM requests rather than a separately maintained figure; older episodes below the attribution boundary keep reporting their persisted per-stage columns. Attribution covers a scoring request the moment the article is polled (before any episode exists to charge it to), so an episode's score total is projected only once every article it was charged for has its scoring request linked back to it; otherwise the persisted columns are used. A failed run's cost is never dropped: an episode that fails after issuing requests still reports what it was charged, whether it is retried under the same episode or re-run as a new one. The Research row includes the cost of the `research-plan` call alongside the web-search cost. LLM cost comes from the provider's own reported charge wherever one is available (OpenRouter returns the exact cost of every call, and a cached call replays the cost of the original), falling back to the per-model rates configured in `application.yaml` for calls that report nothing. Each episode and each request records where its cost came from, so an actual charge is distinguishable from an estimate, including the mixed case where only some stages reported one. Stage costs are tracked with sub-cent precision, so a stage that costs a fraction of a cent is not rounded away to zero. The Jev call that gates **Dedup** runs on a different model at a different rate and reports its own charge, so it is a row of its own beside the call it relieves rather than folded into it, naming the model it is configured to run on for any episode whose gate actually issued requests. The dashboard renders this breakdown in a dedicated **Costs** tab on the episode detail page.

Every LLM request is also logged on its own, with the time it took, the tokens it used, its resolved cost and cost source, the upstream provider that actually served it (useful behind a router like OpenRouter), its reasoning token count where the provider reports one, and whether it succeeded, was served from the cache, or failed. The Jev decision calls are logged the same way, so the gate is visible beside the stages it precedes. The API reports p50, p90, p95 and p99 per stage over a requested window together with the sample count each was computed from, and the dashboard shows it in a **Latency** tab, slowest stage and slowest request first; the `research-plan` stage appears here too. For OpenRouter requests, OpenRouter's generation stats are fetched shortly after each request and split its time into startup, reasoning and writing, with output speed and every provider OpenRouter tried; the episode's request list shows these per request. Percentiles cover successful requests and timeouts and leave out cache hits and faster failures, because a request that was refused quickly says nothing about how long an answer takes. These request rows are pruned after a configurable retention period, which is why an episode's own costs live on the episode rather than being recomputed from them. Pricing is configured per model in `application.yaml`; see [docs/configuration.md#model-configuration](docs/configuration.md#model-configuration). Before any LLM call, a **cost gate** estimates the total spend and skips the run if it would exceed a configurable threshold (`maxLlmCostCents` per podcast, or the global `app.llm.max-cost-cents`). The `EVAL` stage is deliberately absent from this breakdown: judging happens after the episode exists and may happen many times, so its cost is recorded on the score row instead. Folding it in would corrupt both the per-episode economics and the cost gate.

## Deep-Dive Web Research

When `deepDiveEnabled` is set on a podcast, a pre-compose research stage plans up to 3 [Tavily](https://tavily.com) searches per regular episode (a planning call turns the day's topics into search queries; an unparseable plan falls back to searching the topics themselves) and hands the results to the script composer as a "Background research" block in its prompt. A focus episode always researches, regardless of `deepDiveEnabled`, with a budget of up to 5 searches. Searches run concurrently, are cached, and a failed or timed-out search contributes no results without failing the episode. See [docs/deep-dive-research.md](docs/deep-dive-research.md) for configuration and API key resolution.

## Focus Episodes

Besides the regular scheduled episode, you can generate a one-off **focus episode** on demand: on the Upcoming Episode page, press "Generate Episode", choose **Focus** in the dialog that opens (instead of **Regular**) and type a topic (e.g. "Claude Opus 5.5 release"). The dialog explains what each kind of episode triggers. A focus episode is scored against that free-text focus instead of the podcast's topic, selecting from the same current article window as a regular manual generation, and considers every article in that window regardless of relevance to the podcast's general topic or whether a regular episode already used it (pure retweets are excluded; replies and quote posts with their own text are kept). If nothing clears the relevance threshold against the focus text, generation fails with a message naming the focus. Web search always runs for a focus episode, whatever the podcast's deep-dive setting, with up to 5 queries (instead of 3) spread over distinct angles of the subject: the announcement itself, reactions, numbers or benchmarks, and what it means for the field.
