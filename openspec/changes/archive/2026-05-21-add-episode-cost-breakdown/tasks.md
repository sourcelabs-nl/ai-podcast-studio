...
- [x] 7.4 Live run: trigger generation on a real podcast; open the new Costs tab on the resulting episode; verify all six rows are populated with non-zero values and the Total equals the sum
- [x] 7.5 Run `openspec validate add-episode-cost-breakdown --strict`
- [x] 7.6 Update `README.md` to mention the new Costs tab (optional, light touch)
