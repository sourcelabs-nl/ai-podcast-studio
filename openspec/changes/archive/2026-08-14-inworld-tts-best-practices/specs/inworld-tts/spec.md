...
#### Scenario: Non-verbal tag spelling matches the Inworld sound names
- **WHEN** `scriptGuidelines()` is called for any style
- **THEN** the returned text lists `[clear throat]` and does NOT list `[clear_throat]`

#### Scenario: Guidelines describe steering instructions
- **WHEN** `scriptGuidelines()` is called for any style
- **THEN** the returned text explains that a single bracketed English delivery instruction may open a turn or segment, and that `[reset]` clears it

#### Scenario: Guidelines describe acronym handling
- **WHEN** `scriptGuidelines()` is called for any style
- **THEN** the returned text instructs the LLM to expand an acronym on first use and to spell out unpronounceable acronyms

#### Scenario: Guidelines allow SSML break tags with their limits
- **WHEN** `scriptGuidelines()` is called for any style
- **THEN** the returned text shows the `<break time="1s" />` form and states the 20-per-request and 10-second limits

#### Scenario: Guidelines describe CAPS emphasis
- **WHEN** `scriptGuidelines()` is called for any style
- **THEN** the returned text describes capitalising a whole word or a single syllable as an emphasis option alongside `*word*`
...
- **WHEN** `scriptGuidelines(PodcastStyle.CASUAL, mapOf("Anthropic" to "/aenˈθɹɒpɪk/"))` is called
- **THEN** the returned text includes a "Pronunciation Guide" section with the entry and instructs the LLM to REPLACE the word with its IPA phoneme, and to ONLY use IPA for listed terms
