...
- [x] 2.1 Add an `isSpeakerOpening: Boolean` parameter to `InworldTtsProvider.prepareChunks` and document that it marks the request sent with no preceding audio in that voice
...
- [x] 2.3 Pass `isSpeakerOpening = true` from `generateMonologue`, and track the roles already seen in `generateDialogue` so each role's first turn passes `true`

## 3. Composer guidance

- [x] 3.1 Add a `CORE_GUIDELINES` bullet forbidding a delivery direction on a speaker's very first turn, with the reason (no preceding audio in that voice to anchor the cue) and both observed failures
...
- [x] 4.2 `InworldTtsProviderTest`: monologue opening stripped, every role's first dialogue turn stripped, and a role's later turn keeping its instruction, leading sound tag preserved
...
- [x] 4.5 Update `steering instructions do not leak across dialogue turns` to set its cue on a role's second turn, so it still exercises leakage now that every role's first turn is stripped
- [x] 4.6 Run `mvn test` and confirm the full suite passes (1413 tests, 0 failures)
