...
**8. The experiment cost guard is a configured `app.experiments.max-variant-repeats` (variants x repeats, default 6), checked in `ExperimentService.startExperiment` before any episode is created or any `PipelineRunner.run` call is made.** A hard cap rather than a soft warning, per the proposal's explicit call for one; an oversized request is a 400 (`experiment_too_large`). Six covers two variants x three repeats.

**9. Experiments are served by `ExperimentController` at `/users/{userId}/podcasts/{podcastId}/episodes/{episodeId}/experiments`.** `POST` takes `{variants: [{name, models, reasoningEffort, providerSort, preferredMinThroughput, targetWords, researchQueryCap, bypassLlmCache}], repeats, includeBaseline}` (stage maps keyed by `PipelineStage.value`), and `ExperimentMappers.toPlan` turns it into an `ExperimentPlan` of `ExperimentVariant(name, RunOverrides)`; `includeBaseline` adds a variant named `baseline` with no overrides. It returns 202 with the experiment id and the placeholder episode ids. An episode without linked articles, or an experiment episode, is refused with 409.

  Each run gets its own placeholder episode, created up front with purpose `EXPERIMENT` by `EpisodeService.createExperimentEpisode`, which neither moves the schedule nor emits `episode.generating`. The runs execute one after another on one background coroutine, interleaved by repeat (every variant once, then again), so a provider's load at one moment does not favour one variant and the compose durations stay comparable. A variant that leaves `bypassLlmCache` unset bypasses the LLM cache, because otherwise every repeat after the first replays the first one's script.

  *Alternative considered*: launching all runs at once. Rejected: concurrent runs contend for the same providers and the SQLite writer, which distorts exactly the duration figures the comparison reports.

**10. An experiment episode records its grouping in three nullable columns (`V83__add_episode_experiment_identity.sql`): `experiment_id` (one POST), `experiment_variant` and `experiment_source_episode_id` (indexed, `ON DELETE SET NULL`).** Columns rather than a key in `run_config_json`, because the comparison queries by source episode and groups by experiment and variant, and a separate experiments table would hold nothing the episodes do not already carry.

**11. The comparison (`GET` on the same path) returns every experiment against the source episode, per variant and per run:** status, the latest judge score and judge model, total cost (compose, research and recap; the judge is not billed to the episode) and compose cost in fractional cents (provider-reported where present), the compose wall time (first compose request's start to the last one's end, from `llm_calls`), the compose call count, the summed reasoning tokens, the distinct serving providers and the spoken word count (`ScriptMetrics`, falling back to plain words for a script without speaker tags). Each variant also reports its completed, failed and pending run counts and the mean of every figure over its completed runs.

**12. Closed-weight vendor models are routed without the quantization floor.** OpenRouter's `provider.quantizations` filter excludes an endpoint whose quantization is "unknown", and every endpoint of a closed-weight model (the vendor itself, Azure, Bedrock) reports "unknown", so the floor made `openai/*` and `anthropic/*` models unroutable (404 "No endpoints found"). `OpenRouterRouting.extraBodyFor` takes the model name and omits `quantizations` for the prefixes in `CLOSED_WEIGHT_PREFIXES` (`openai/`, `anthropic/`), keeping `require_parameters`, the reasoning block and any provider preferences. No third party can serve a lossy copy of a closed-weight model, so the floor protects nothing there. `google/` is not on the list because Gemma is open-weight and third parties quantize it. Every open-weight model sends exactly the body it sent before.

  OpenAI's reasoning models also reject `temperature`, which every stage sets, and with `require_parameters` a request carrying one matches no endpoint either. `withRoutingAndReasoning(model)` therefore clears the temperature for the prefixes in `NO_TEMPERATURE_PREFIXES` (`openai/`), in the one place every stage already passes through; Anthropic models keep theirs.
...
- None open for Phase C's implementation; the two A/B experiments (C.5, C.6) are still to be run.
