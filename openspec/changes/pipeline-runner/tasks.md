...
- [ ] B.6 Add `overrides: RunOverrides?` to `RunSpec` and implement the `Sandbox` outcome (no TTS, never publishes, no `lastGeneratedAt` update, no article consumption, epilogue still runs), and verify with a unit test covering each of those four guarantees
