# llm-generation-stats Specification

## Purpose
Fetches OpenRouter's server-side account of each request after it returned and splits the request's
time into startup, reasoning and writing, so a slow request can be explained and experiments can
compare where compose time goes.
