...
fun buildHistoryGuidanceBlock(research: PreComposeResearch): String {
    val whatCountsAsNew = """
            - WHAT COUNTS AS NEW: The `[FOLLOW-UP: ...]` headers above the article groups are authoritative. An article group carrying such a header continues a story from an earlier episode, and you MUST say so when you introduce it: one short line in your own words telling the listener the show covered this before, then spend the time on what is new. Base that line only on the header, and never invent when it aired or what was said. An article group with NO header is new: cover it as news, and keep it eligible to lead the episode"""
