 ...
 * controller (Rule SB8). Scoped to [PodcastController] and [EpisodeController], and deliberately
 * handles only the specific types below: no catch-all handler, so every other error path keeps the
 * behaviour it has today.
 */
@RestControllerAdvice(assignableTypes = [PodcastController::class, EpisodeController::class])
